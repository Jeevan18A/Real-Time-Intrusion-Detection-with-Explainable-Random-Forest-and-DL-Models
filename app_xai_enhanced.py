...
def flow_detail():
    ...
    try:
        X = np.array(flow.values[0,1:40], dtype=float).reshape(1, -1)
        choosen_instance = X
        
        # --- Prediction Probabilities Table ---
        pred_table_html = ""
        feature_value_html = ""
        class_labels = getattr(classifier, "classes_", ["Benign", "DoS", "DDoS", "FTP-Patator", "Other"])
        
        if classifier is not None:
            proba_score = predict_fn_rf(choosen_instance)
            risk_proba = float(np.sum(proba_score[0][1:]))
            # Build prediction probabilities table
            pred_table_html = "<table border='1'><tr><th>Class</th><th>Probability</th></tr>"
            for idx, label in enumerate(class_labels):
                prob = round(proba_score[0][idx], 2) if idx < proba_score.shape[1] else 0.0
                pred_table_html += f"<tr><td>{label}</td><td>{prob}</td></tr>"
            pred_table_html += "</table>"
        else:
            risk_proba = 0.0
        ...
        if explainer is not None:
            try:
                instance_to_explain = choosen_instance[0]
                exp = explainer.explain_instance(instance_to_explain, predict_fn_rf, num_features=6, top_labels=1)
                exp_html = exp.as_html()
                # --- Feature-Value Table ---
                feature_value_html = "<table border='1'><tr><th>Feature</th><th>Value</th></tr>"
                for feat_idx, feat_val in enumerate(instance_to_explain):
                    if feat_idx < len(cols[1:40]):
                        feature_value_html += f"<tr><td>{cols[feat_idx+1]}</td><td>{round(feat_val, 2)}</td></tr>"
                feature_value_html += "</table>"
            except Exception:
                exp_html = ""
        ...
        return render_template(
            ...
            exp=exp_html,
            feature_table=feature_value_html,
            ae_plot=plot_div,
            risk=risk
        )
    except Exception as e:
